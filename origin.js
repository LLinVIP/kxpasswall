...
function getCountryGroupNames(e, t) {
  const o = e.filter((e) => e.nodes.length >= t);
  return (
    o.sort(
      (e, t) =>
        (countriesMeta[e.country]?.weight ?? 1 / 0) -
        (countriesMeta[t.country]?.weight ?? 1 / 0),
    ),
    o.map((e) => e.country + "节点")
  );
}
...
function buildBaseLists({ landing: e, lowCostNodes: t, countryGroupNames: o }) {
  const r = t.length > 0 || regexFilter,
    n = buildList(
      PROXY_GROUPS.FALLBACK,
      e && PROXY_GROUPS.LANDING,
      o,
      r && PROXY_GROUPS.LOW_COST,
      PROXY_GROUPS.MANUAL,
      "DIRECT",
    );
  return {
...
const dnsConfig = buildDnsConfig({ mode: "redir-host" }),
  ...
  LOW_COST_REGEX = /0\.[0-5]|低倍率|省流|大流量|实验性/i,
  LANDING_REGEX = /家宽|家庭|家庭宽带|商宽|商业宽带|星链|Starlink|落地/i,
  LANDING_PATTERN = "(?i)家宽|家庭|家庭宽带|商宽|商业宽带|星链|Starlink|落地";
function parseLowCost(e) {
  return (e.proxies || [])
    .filter((e) => LOW_COST_REGEX.test(e.name))
    .map((e) => e.name);
}
function parseLandingNodes(e) {
  return (e.proxies || [])
    .filter((e) => LANDING_REGEX.test(e.name))
    .map((e) => e.name);
}
function parseCountries(e) {
  ...
  for (const [e, t] of Object.entries(countriesMeta))
    r[e] = new RegExp(t.pattern.replace(/^\(\?i\)/, ""));
  for (const e of t) {
    const t = e.name || "";
    if (!LANDING_REGEX.test(t) && !LOW_COST_REGEX.test(t))
  ...
  const n = [];
  for (const [e, t] of Object.entries(o)) n.push({ country: e, nodes: t });
  return n;
}
function buildCountryProxyGroups({
  ...
  const s = [],
    l = "0\\.[0-5]|低倍率|省流|大流量|实验性",
    i = LANDING_PATTERN,
    a = o ? "load-balance" : "url-test",
    c = r ? null : Object.fromEntries(n.map((e) => [e.country, e.nodes]));
  for (const n of e) {
    ...
    if (r)
      p = {
        ...
        "include-all": !0,
        filter: e.pattern,
        "exclude-filter": t ? `${i}|${l}` : l,
        type: a,
      };
    else {
      const t = c[n] || [];
      p = { name: `${n}节点`, icon: e.icon, type: a, proxies: t };
    }
    (o ||
      Object.assign(p, {
...
function buildProxyGroups({
  ...
  landingNodes: n,
  defaultProxies: s,
  defaultProxiesDirect: l,
  defaultSelector: i,
  defaultFallback: a,
}) {
  const c = t.includes("台湾"),
    p = t.includes("香港"),
    u = t.includes("美国"),
    d = e
      ? i.filter(
          (e) => e !== PROXY_GROUPS.LANDING && e !== PROXY_GROUPS.FALLBACK,
        )
      : [];
  return [
    {
    ...
    e
      ? {
          ...
          type: "select",
          ...(regexFilter
            ? { "include-all": !0, filter: LANDING_PATTERN }
            : { proxies: n }),
        }
      : null,
    {
    ...
    },
    {
      ...
      type: "select",
      proxies: e ? buildList(PROXY_GROUPS.SELECT, PROXY_GROUPS.LANDING, ...s.slice(1)) : s,
    },
    {
      name: "Crypto",
    ...
    },
    {
      ...
      type: "select",
      proxies: c && p ? [PROXY_GROUPS.DIRECT, "台湾节点", "香港节点"] : l,
    },
    {
      name: "Bahamut",
    ...
    },
    {
      ...
      type: "select",
      proxies: u ? ["美国节点", PROXY_GROUPS.SELECT, PROXY_GROUPS.MANUAL] : s,
    },
    {
      name: "OneDrive",
...
function main(e) {
  const t = { proxies: e.proxies },
    o = parseCountries(t),
    r = parseLowCost(t),
    n = landing ? parseLandingNodes(t) : [],
    s = getCountryGroupNames(o, countryThreshold),
    l = stripNodeSuffix(s),
    {
      defaultProxies: i,
      defaultProxiesDirect: a,
      defaultSelector: c,
      defaultFallback: p,
    } = buildBaseLists({
      landing: landing,
